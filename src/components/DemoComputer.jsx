/*
  ...
  This file is based on a GLTF model optimized for use in React Three Fiber.
*/

import { useRef, useEffect } from 'react'; // React hooks for referencing elements and side effects
import { useGLTF, useAnimations, useVideoTexture } from '@react-three/drei'; // Drei utilities for 3D models, animations, and video textures
import gsap from 'gsap'; // GSAP animation library
import { useGSAP } from '@gsap/react'; // React integration with GSAP animations

const DemoComputer = (props) => {
  const group = useRef(); // Reference to the 3D group object (the entire computer model)

  // Load the GLTF model, materials, and animations from the 'computer.glb' file
  const { nodes, materials, animations } = useGLTF('/models/computer.glb');

  // Hook to control and access animations for the GLTF model
  const { actions } = useAnimations(animations, group);

  // Load a video texture, either passed as a prop or using a default path
  const txt = useVideoTexture(props.texture ? props.texture : '/textures/project/project1.mp4');

  // Ensure the video texture's Y-axis is correctly oriented
  useEffect(() => {
    if (txt) {
      txt.flipY = false; // Correct the Y-axis flip for the video texture
    }
  }, [txt]); // Re-run this effect when the video texture changes

  // GSAP animation hook for animating the rotation of the model when it first loads
  useGSAP(() => {
    gsap.from(group.current.rotation, {
      y: Math.PI / 2, // Rotate the model from 90 degrees on the Y-axis
      duration: 1, // Duration of the animation (1 second)
      ease: 'power3.out', // Easing for a smooth, natural deceleration
    });
  }, [txt]); // Animation is triggered when the texture (txt) is available

  return (
    // The root group element containing the computer model
